...
describe("MainSystem <-> ClusterSystem end-to-end debug", function () {
  ...
  async function deployFixture() {
    ...
    const mainSystem      = await MainSystem.deploy(
                              await topicFactory.getAddress(),
                              await entryPoint.getAddress()
                            );
    await mainSystem.waitForDeployment();

    /* TopicFactory에 MainSystem 주소 갱신 (constructor에서 setter 미리 열어 두었다고 가정) */
    await topicFactory.setMainSystem(await mainSystem.getAddress());

    /* 4) 리턴. 이후 테스트 케이스에서 구조분해할 변수들 모아 두기 */
    return { deployer, alice, bob, entryPoint, topicFactory, mainSystem };
  }
  ...
  it("creates cluster and records mapping", async function () {
    const { alice, mainSystem } = await loadFixture(deployFixture);

    /* 정책 다이제스트(dummy) 준비 */
    const policyDigest = ethers.keccak256(ethers.toUtf8Bytes("initial-policy"));

    /* alice가 클러스터 생성 (0.003ETH 예치) */
    const tx = await mainSystem
      .connect(alice)
      .createCluster(policyDigest, { value: ethers.parseEther("0.003") });

    const receipt    = await tx.wait();
    const eventLog   = receipt!.logs.find(
                         l => (l as any).fragment?.name === "ClusterCreated"
                       ) as unknown as ethers.EventLog;

    const clusterId  = eventLog.args[0];
    const clusterAddr= eventLog.args[1];

    /* 매핑 검사 */
    expect(await mainSystem.clusterIdToAddrs(clusterId)).to.equal(clusterAddr);
    expect(await mainSystem.addressToClusterId(clusterAddr)).to.equal(clusterId);
  });
  ...
  it("grants roles to members", async function () {
    const { alice, bob, mainSystem } = await loadFixture(deployFixture);

    /* 1단계: 클러스터 먼저 생성 */
    const digest = ethers.keccak256(ethers.toUtf8Bytes("policy"));
    const tx  = await mainSystem.connect(alice)
  ...
  it("blocks and unblocks a user", async function () {
    ...
    /* 클러스터 준비 */
    const digest = ethers.keccak256(ethers.toUtf8Bytes("policy"));
    const cr   = await mainSystem.connect(alice)
                  .createCluster(digest, { value: ethers.parseEther("0.003") });
    const lg   = (await cr.wait())!.logs.find(
                   l => (l as any).fragment?.name === "ClusterCreated"
                 ) as unknown as ethers.EventLog;
    const cluster = await ethers.getContractAt("ClusterSystem", lg.args[1]);
  ...
  it("reserves and finalizes gas cost via Paymaster", async function () {
    ...
    const tx  = await mainSystem.connect(alice).createCluster(
                   ethers.keccak256(ethers.toUtf8Bytes("policy")),
                   { value: ethers.parseEther("0.005") }
                 );
    const lg  = (await tx.wait())!.logs.find(
                  l => (l as any).fragment?.name === "ClusterCreated"
                ) as unknown as ethers.EventLog;
